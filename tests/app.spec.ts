...
test('app: loads the shell and renders the sidebar', async ({ page }) => {
  await page.goto('/');
  // Must track the app name in app/index.html — this assertion silently went stale when the
  // app was renamed from "Notes" to "Cyberdyne Systems", which failed every PR's CI.
  await expect(page).toHaveTitle(/Cyberdyne Systems/);
