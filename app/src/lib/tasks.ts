import type { NoteFile, ParsedTask, TaskPriority } from '../types';

export const TASK_MANAGER_ID = 'tasks-main';
...
export function scanTasks(files: NoteFile[], sources: Record<string, string>): ParsedTask[] {
...
export function inNextDays(due: string | undefined, days: number): boolean {
  if (!due) return false;
  const diff = toLocalDate(due).getTime() - toLocalDate(todayISO()).getTime();
