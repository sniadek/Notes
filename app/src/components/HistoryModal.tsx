    ...
    <div onClick={close} style={{ position: 'fixed', inset: 0, background: 'rgba(30,28,24,.28)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, animation: 'fade .12s ease' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: 760, maxWidth: '94vw', height: 560, maxHeight: '88vh', background: 'var(--bg-surface)', borderRadius: 14, boxShadow: 'var(--shadow-modal)', border: '1px solid var(--border)', overflow: 'hidden', animation: 'pop .14s ease', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
          <div className="sc" style={{ width: 230, borderRight: '1px solid var(--border)', overflow: 'auto', padding: 9, flex: 'none' }}>
            {entries.map((h) => (
              <div
                key={h.idx}
                ...
                ? diffRows.map((d, i) => (
                  <div
                    ...
                    style={{
                      ...
                      color: d.t === 'add' ? '#157a44' : d.t === 'del' ? '#b0392b' : 'var(--text-muted)',
                    }}
                  >
                    {(d.t === 'add' ? '+ ' : d.t === 'del' ? '− ' : '  ') + d.v}
                  </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 16px', borderTop: '1px solid var(--border)' }}>
              ...
              <span
                ...
                style={{
                  ...
                  background: snap ? 'oklch(0.5 0.12 var(--accent-hue))' : '#ece9e2', color: snap ? '#fff' : 'var(--text-faintest)',
